document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function saveOrConfirmOrder(status) {
        const total = cart.reduce((acc, item) => acc + (item.precioUnitario * item.unidades), 0); // Calcular total del pedido

        // El ID del pedido debe incluir el Código del Vendedor
        const pedidoIdCompleto = `${vendedorCodigo}-${orderId}`;

        const order = {
            id: pedidoIdCompleto,
            clientId: selectedClient._id || selectedClient.id, // Ajustar para usar ID correcto según estructura JSON
